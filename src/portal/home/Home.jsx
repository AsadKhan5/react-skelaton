...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const filteredData = simData.filter((sim) => {
      ...
      return matchSearchParam1 && matchSearchParam2;
    });

    setFilteredSimData(filteredData);
  }, [simData, searchParam1, searchValue1, searchParam2, searchValue2]);
  ...
  return (
    <div className="flex flex-col gap-10 p-2">
      <h1 className="font-semibold text-xl">Sim Data Table</h1>
      <div className="flex gap-2">
        <div className="flex join-item">
          <select
            value={searchParam1}
            onChange={(e) => setSearchParam1(e.target.value)}
            className="search-select text-sm border rounded-l-md"
          >
          ...
          <select
            ...
            className="search-select border text-sm"
          >
            <option value="">Search Type</option>
            <option value="ICCID">ICCID</option>
            <option value="IMSI">IMSI</option>
          ...
          <input
            ...
            className="search-input border"
          />
          <button className="btn btn-secondary btn-sm rounded-none rounded-r-md cursor-none">
            <MdManageSearch className="text-3xl" />
          </button>
        </div>
        {/* pagination  */}
        <div className="flex join fixed right-4">
          <select
            value={count}
